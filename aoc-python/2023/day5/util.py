...
class Point:
    ...
    def __add__(self, other):
        return Point(self.x + other.seed, self.y + other.y)
    ...
    def __eq__(self, other):
        return other and self.x == other.seed and self.y == other.y
    ...
    def manhattan_distance(p1, p2):
        return abs(p1.seed - p2.seed) + abs(p1.y - p2.y)
...
INTERCARDINAL_DIRECTIONS = [NORTH_EAST, SOUTH_EAST, SOUTH_WEST, NORTH_WEST]
...
def ints(s, single_digit=False, negatives=False):
    pattern = f"{'-?' if negatives else ''}\\d" f"{'' if single_digit else '+'}"
...
class TestInts(unittest.TestCase):
    ...
    def test_single_digit_negatives(self):
        self.assertEqual(
            ints("123 abc -456", single_digit=True, negatives=True), [1, 2, 3, -4, 5, 6]
        )
